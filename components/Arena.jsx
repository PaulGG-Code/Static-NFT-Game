...
const handleShadowColor = (power) => {
  if (power === "🔥")
    return "0px 0px 10px 1px #E53E3E, 0px 0px 20px 0px #DD6B20, 0px 0px 30px 0px #FBD38D";
  if (power === "💧")
    return "0px 0px 10px 1px #3182CE, 0px 0px 20px 0px #00B5D8, 0px 0px 30px 0px #76E4F7";
  if (power === "🌿")
    return "0px 0px 10px 1px #38A169, 0px 0px 20px 0px #68D391, 0px 0px 30px 0px #9AE6B4";
};
...
const Arena = ({ contract, abi }) => {
  ...
  useEffect(() => {
    ...
    if (ethereum) {
      ...
      const gameContract = new ethers.Contract(contract, abi, signer);
  ...
  const handleBossPower = () => {
    if (characterPower) {
      const randomPowerSelection =
        powers[Math.floor(Math.random() * (powers.length - 0))].name;
  ...
  useEffect(() => {
    animateComponent(cardCharacterRef, "animate__pulse", "0.1s");
    setAttackState(ATTACK_STATE.NULL);
    setShowBossPower(false);
    handleBossPower();
  }, [characterPower]);
  ...
  useEffect(() => {
    if (attackState === ATTACK_STATE.BOSS_HIT) {
      animateComponent(cardBossRef, "animate__bounceOutLeft", "0.1s");
      animateComponent(cardCharacterRef, "animate__jello");
    }
    if (attackState === ATTACK_STATE.CHARACTER_HIT) {
      animateComponent(cardCharacterRef, "animate__bounceOutRight", "0.1s");
      ...
      <Flex align={"center"} justify={"center"} w={"100%"} py={10}>
        ...
        >
          {boss && characterNFT && boss.hp !== 0
            ...
            : "You've defeated the boss! Don't rest for long, soon a new boss will be reborn"}
        </Text>
      </Flex>
      {boss && characterNFT && (
        <Flex
          ...
          <Flex direction={"column"} w={"50%"} p={5}>
            {loader ? (
              ...
              boss.hp !== 0 && (
                ...
                >
                  <Flex w={"100%"} align={"center"} justify={"center"} mb={10}>
                    {renderBtns()}
                  </Flex>
                  <Flex
                    w={"100%"}
                    ...
                    >
                      {powers.map((btn) => (
                        <Button
                          ...
                          variant={"outline"}
                          disabled={
                            !showBossPower ? true : bossPower !== btn.name
                          }
                          filter={
                            (!showBossPower || bossPower !== btn.name) &&
                            "grayscale(100%)"
                          }
                          boxSize={20}
                          mb={5}
          ...
          {boss && (
            <div ref={cardBossRef}>
              <ArenaCard
                ...
                boxShadow={
                  showBossPower &&
                  `0px 0px 30px 0px ${handleShadowColor(bossPower)}`
                }
                style={{
